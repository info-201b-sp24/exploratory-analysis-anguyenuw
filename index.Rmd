...
To collect data on player scores, I first found the first and last tournament matches held in 2023. These are approximately match ID [105988619](https://osu.ppy.sh/community/matches/105988619) and match ID [111977659](https://osu.ppy.sh/community/matches/111977659). Then, I queried the osu! API for every match ID in this range, and filtered out all multiplayer games that weren't a tournament match. I then extracted all match data 
...
A large problem that I will need to address is the fact that not all maps played in a tournament match are actually part of the match. Players will often be allowed to play a "warm-up" map before the match, which is still logged in the match history. Players can also play maps after the match is over, which is also logged in the match history. I will need to somehow clean both of these from my dataset.
...
- Separating tournament matches from other multiplayer matches
