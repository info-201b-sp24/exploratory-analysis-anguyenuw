...
f <- read.csv("csvfiles/p02dataset.csv")
...
```{r}
scores_raw 
scores_raw <- read.csv("csvfiles/p02dataset.csv") %>%
          filter(!is.na(User.ID)) %>%
          distinct()
...
for (map_shard in scores) {
  while (nrow(map_shard) > 0) {
    ...
    players_and_scores[1, 1] <- time
    players_and_scores[1, 2:(1 + n)] <- map_scores$User.ID
...
# see scraper.py/gather_players

player_data <- read.csv("csvfiles/user_data - Copy.csv")
ratings_with_userid <- player_ratings


# fill in RME ratings
player_data <- player_data %>%
  left_join(ratings_with_userid, by = join_by(Username == Player)) %>%
  mutate(RME = Rating) %>%
  select(UserID, Username, GlobalRank, PP, RME, Location)

player_data %>% select(UserID, Username, GlobalRank, PP, RME, Location)
```
